...
function openGame(id){current=games.find(g=>g.id===id);$('gameTitle').textContent=current.name;$('slot').classList.remove('hidden')}
function closeSlot(){$('slot').classList.add('hidden')}
async function spin(){try{let d=await api('/api/spin',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({gameId:current.id,bet:$('bet').value})});d.reels.forEach((x,i)=>$('r'+i).textContent=x);user=d.user;$('balance').textContent=money(user.balance);$('userBox').innerHTML=`${user.name} • 🪙 <b>${money(user.balance)}</b> <button onclick="logout()">Sair</button>`;$('msg').textContent=d.prize?`🎉 +${money(d.prize)} créditos`:'Não foi dessa vez.';loadHistory()}catch(e){$('msg').textContent=e.message}}
async function bonus(){try{let d=await api('/api/demo/bonus',{method:'POST',headers:{'Content-Type':'application/json'}});user=d.user;$('balance').textContent=money(user.balance);alert('Bônus demo adicionado.')}catch(e){alert(e.message)}}
function logout(){localStorage.removeItem('ns_token');location.reload()}
if(token)init();else $('auth').classList.remove('hidden');
